...
public class Bullet extends Entity implements IRenderableObject {
	...
	public synchronized void render(GraphicsContext gc) {
		...
		gc.setLineWidth(2);
		gc.fillOval(this.getX() - this.getRadius(), this.getY() - this.getRadius(), this.getRadius() * 2,
				this.getRadius() * 2);
		gc.strokeOval(this.getX() - this.getRadius(), this.getY() - this.getRadius(), this.getRadius() * 2,
				this.getRadius() * 2);
