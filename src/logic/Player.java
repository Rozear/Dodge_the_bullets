...
public class Player extends CollidableEntity implements IRenderableObject {
	...
	public Player(float x, float y, double angle) {
		...
		// Player.PLAYER_BUFFED_PATTERN = new SpreadPattern(this, 5, 30, 1, 0,
		// 50);
					...
					for (int i = 0; i < 5; i++) {
						for (double angle = -Math.PI / 12; angle <= Math.PI / 12; angle += Math.PI / 48) {
							Main.logic.addNewObject(
									new Bullet(owner.getX(), owner.getY(), owner.getAngle() + angle, 10, 5, 1, owner));
						}
						Thread.sleep(50);
					}
	...
	public void shoot() {
		// TODO Auto-generated method stub
		if (Main.logic.getPlayer().isDestroy())
			return;
	...
	void update() {
		...
		if (!this.isDestroy()) {
			...
			if (valueX != 0 && valueY != 0) {
				this.setX((float) (this.getX() + valueX * this.getSpeed() * Math.cos(Math.PI / 4)));
				this.setY((float) (this.getY() + valueY * this.getSpeed() * Math.sin(Math.PI / 4)));
			} else {
			...
			if (this.getX() + this.getRadius() > Configuration.ARENA_WIDTH)
				this.setX(Configuration.ARENA_WIDTH - this.getRadius());
			if (this.getY() - this.getRadius() < 0)
				this.setY(0 + this.getRadius());
			if (this.getY() + this.getRadius() > Configuration.ARENA_HEIGHT)
				this.setY(Configuration.ARENA_HEIGHT - this.getRadius());
			...
			if (InputUtility.isMouseLeftDown()) {
				// DrawingUtility.flashScreen(((Canvas)
				// Main.gameScreen.getChildren().get(0)).getGraphicsContext2D());
				this.shoot();
			}
