...
public abstract class PlayerSkill extends Canvas {
			...
			public void spawnBullet() {
				...
				try {
					for (int i = 0; i <= wave; i++) {
						Main.logic.addNewObject(new Bullet((float) (x + i * 25 * Math.cos(angle)),
								(float) (y + i * 25 * Math.sin(angle)), angle + Math.PI / 2, Bullet.DEFAULT_SPEED, 20,
								2, owner));
						Main.logic.addNewObject(new Bullet((float) (x + i * 25 * Math.cos(angle)),
								(float) (y + i * 25 * Math.sin(angle)), angle - Math.PI / 2, Bullet.DEFAULT_SPEED, 20,
								2, owner));
	...
	public void render() {
		...
		if (currentCD > 0) {
			...
			gc.fillArc(-this.getWidth() * 0.5, -this.getHeight() * 0.5, this.getWidth() * 2, this.getHeight() * 2, 90,
					(currentCD) * 360 / skillCD, ArcType.ROUND);
			gc.setGlobalAlpha(1);
		}
